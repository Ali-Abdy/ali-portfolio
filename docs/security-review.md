...
Reviewed on 26 September 2026. Scope: this portfolio repository, its locally available Git history, locked dependencies, public assets, production build, and local HTTP/browser behavior. The starting commit was `791e3df` on `portfolio-redesign-review`.
...
| ------------------------ | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | -------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| Critical dependency      | Next.js 16.3.5 fell within the affected range for the 22 September 2026 Node.js `next/og` `ImageResponse` remote-code-execution advisory. The portfolio uses `ImageResponse` for social cards, although its route is static and accepts only fixed typed content. | Upgraded and locked Next.js and `eslint-config-next` to 16.3.6, the official patched Active LTS release. The social route remains build-time static, locale-restricted, and free of visitor input.                                                                                                                   |
...
- Final local checks passed: lint, TypeScript, production build, and all 29 Playwright tests (including 7 security regression tests and legal-route publication-state checks). No ordinary-404 framework errors appeared in the final test run.
...
- Installed Next.js 16.3.6 is the official patch for the 22 September 2026 Node.js `ImageResponse` advisory affecting `>=16.2.0 <16.3.6`. The social-card route was not visitor-controlled, but the dependency was still patched rather than relying on that constraint.
...
1. Choose the host and final domain. Confirm supported Node.js, HTTPS, forced HTTPS redirects, the response headers above, caching, and image limits at the actual edge. Set `SITE_URL` and rebuild. For Vercel production, the build now refuses to proceed without `SITE_URL`, enabled legal pages with complete server-side legal contact values, and an explicit privacy-notice review gate. Do not expose a development server.
...
- [Next.js September 2026 `ImageResponse` security update](https://nextjs.org/blog/nextjs-security-update-september-22-2026)
